...
const handleSubmit = async () => {
  ...
  if (!reg_email.test(email.value)) {
    ...
    try {
      ...
      if (!exist) {
        message.success(t('successMessage.emailSuccess'))

      } else {
        message.warning(t('warnMessage.duplicateEmail'))
      }
    } finally {
      loading.value = false
    }
...
<template>
  <div class="login-view flex flex-col lg:w-100% lg:flex-row">
    <div class="login-left min-h-40vh w-100% flex flex-col lg:min-h-100vh lg:w-50vw lg:flex-row">
      ...
      <div class="flex flex-full items-center justify-center lg:justify-end lg:pr-20">
